...
void main() {
  ...
  group('PlacesService', () {
    ...
    test(
      'getPredictions uses correct endpoint and includes session token',
      () async {
        when(
          () => mockDio.post(
            any(),
        ...
        final results = await placesService.getPredictions(
          'Muscat',
          sessionToken,
        );
        ...
        verify(
          () => mockDio.post(
            ...
            data: any(
              named: 'data',
              that: containsPair('sessionToken', sessionToken),
            ),
            options: any(named: 'options'),
          ),
        ).called(1);
