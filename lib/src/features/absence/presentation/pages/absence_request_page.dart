...
class _AbsenceRequestPageState extends State<AbsenceRequestPage> {
  ...
  final List<Map<String, String>> _periods = [
    {'id': 'full_day', 'label': 'fullDay'},
    {'id': 'morning', 'label': 'morningOnly'},
    {'id': 'afternoon', 'label': 'afternoonOnly'},
  ];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Absence Period (New)
            _buildSectionHeader(context.t('absencePeriod')),
            const SizedBox(height: 12),
            Wrap(
              spacing: 10,
              runSpacing: 10,
              children: _periods
                  .map(
                    (p) => _buildChoiceChip(
                      label: context.t(p['label']!),
                      selected: _absencePeriod == p['id'],
                      onSelected: (val) =>
                          setState(() => _absencePeriod = p['id']!),
                    ),
                  )
                  .toList(),
            ),
            ...
            // Absence Reason
            _buildSectionHeader(context.t('reason')),
            const SizedBox(height: 12),
            Wrap(
              spacing: 12,
              children: _reasons
                  .map(
                    (t) => _buildChoiceChip(
                      label: context.t(t['label']!),
                      selected: _absenceType == t['id'],
                      onSelected: (val) =>
                          setState(() => _absenceType = t['id']!),
                    ),
                  )
                  .toList(),
            ),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: context.t('optionalWriteDetails'),
                filled: true,
      ...
      if (mounted) {
        if (response.success) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                context.t('absenceRequestSentSuccessfully'),
              ),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                response.message ?? context.t('failedToLoadMessages'),
              ),
